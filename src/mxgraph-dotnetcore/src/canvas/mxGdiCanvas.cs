...
namespace com.mxgraph
{
    ...
    public class mxGdiCanvas : mxBasicCanvas
    {
        ...
        /// </summary>
        protected async System.Threading.Tasks.Task<Image> LoadImage(String image)
        {
            Image img = (imageCache.ContainsKey(image)) ? imageCache[image] : null;

            if (img == null)
            {
                img = await mxUtils.LoadImage(image);
        ...
        /// <param name="h">Height of the image.</param>
        /// <param name="image">URL of the image.</param>
        protected async void DrawImage(int x, int y, int w, int h, String image,
            bool preserveAspect, bool flipH, bool flipV)
        {
            Image img = await LoadImage(image);
